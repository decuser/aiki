...
## v0.4.0 Engine Rebuild

### Grammar Contract Architecture
HIST Introduced GrammarContract interface decoupling grammar from parser. [engine/syntax/grammar_contract.go]
HIST Created definition subpackage with embedded EBNF parser. [engine/syntax/definition/parser.go]
HIST Grammar loaded via definition.New returning GrammarContract. [engine/syntax/definition/definition.go]
HIST EBNF grammar.ebnf is source of truth parsed at runtime. [engine/syntax/definition/grammar.ebnf]

### Evaluator Rebuild
HIST Rebuilt evaluator with handler map architecture. [engine/semantics/evaluator/handlers.go]
HIST Handler map keyed by node.Type with ValidateHandlers panic. [engine/semantics/evaluator/handlers.go]
HIST Scope replaces Env with clearer terminology. [engine/semantics/evaluator/scope.go]
HIST Flat value.Value struct with Type enum. [engine/semantics/value/value.go]

### Intrinsics
HIST export intrinsic declares module exports. [engine/semantics/evaluator/intrinsics.go evalExport]
HIST import intrinsic loads modules with export enforcement. [engine/semantics/evaluator/intrinsics.go evalImport]
HIST load intrinsic executes file in current scope. [engine/semantics/evaluator/intrinsics.go evalLoad]
HIST apply intrinsic spreads list as function arguments. [engine/semantics/evaluator/intrinsics.go evalApply]
HIST spawn intrinsic launches concurrent tasks. [engine/semantics/evaluator/intrinsics.go evalSpawn]

### HAL Primitives
HIST Added modulo for remainder division. [engine/runtime/hal/substrate/go_runtime.go]
HIST Added ord for character code. [engine/runtime/hal/substrate/go_runtime.go]
HIST Added shape for type inspection. [engine/runtime/hal/substrate/go_runtime.go]
HIST Print concatenative with no implicit separators. [engine/runtime/hal/substrate/go_runtime.go]

### Grammar Fixes
HIST Fixed repetition patterns with helper productions. [engine/syntax/definition/grammar.ebnf]
HIST arg_tail param_tail field_tail pattern_tail for proper sequence repeats.
HIST infix_tail pipe_tail for operator chains.
HIST Removed == operator per design using equal function. [engine/syntax/definition/grammar.ebnf]
