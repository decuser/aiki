...
The repository-side release-prep work is complete except for final validation that cannot be executed in the sandbox because Go 1.24 is unavailable locally and cannot be fetched. The authoritative Linux release candidate must run `make validate` before the session can be marked COMPLETE and before the final alpha tag/publication step.
