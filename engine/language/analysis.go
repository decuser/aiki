...
// Diagnostic is an editor-independent language finding.
type Diagnostic struct {
	Pos      engine.Position
	Severity string // "error" or "warning"
	Source   string // "lex", "parse", or "lint"
	Message  string
}

// AnalyzeStructure checks source for undefined names, shadowing, and naming conventions.
// It is structural only: it does not evaluate code.
func AnalyzeStructure(g *grammar.Grammar, file string, source string, lintScope value.Scope, catalog Catalog) ([]Diagnostic, error) {
	...
	return analyzeNode(g, file, node, lintScope, catalog), nil
}

func analyzeNode(g *grammar.Grammar, file string, node *syntax.Node, lintScope value.Scope, catalog Catalog) []Diagnostic {
...
func (c *checker) add(level string, pos engine.Position, msg string) {
	c.diags = append(c.diags, Diagnostic{Pos: pos, Severity: level, Source: "lint", Message: msg})
}
...
// resolveModuleExports obtains module source through the catalog contract and
// extracts exports using the real Aiki parser. Resolution is an adapter concern;
// export semantics remain language analysis.
func (c *checker) resolveModuleExports(moduleName string) []string {
	if c.catalog == nil {
		return nil
	}
	_, source, ok := c.catalog.ModuleSource(c.currentFile, moduleName)
	if !ok {
		return nil
	}
